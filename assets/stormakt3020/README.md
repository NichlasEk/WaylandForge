...
`snapphane-wreck-sea-background-v1.png` is Bana 6's opaque Black Wreck Sea plate, packed at both supported widths with a vertically mirrored wrap. `snapphane-wreck-sea-props-v1-source.png` preserves the flat-magenta generation and `snapphane-wreck-sea-props-v1.png` is its alpha-cleaned runtime sheet of eight large inert parallax wrecks. These props render translucent behind every physical mission object and never enter collision or targeting state. Exact built-in prompts and reference roles are stored in the sibling Markdown files.
...
`karl-cclv-radio-v1.png` supplies Karl CCLV's matched neutral/speaking portrait pair: a reference-guided futuristic Carolean officer in worn cobalt armor, brass data clasps and restrained cyan optics. The chroma source and exact prompt are versioned beside it. `karl-silver-scent-sv-radio.wav` introduces his own fully synthetic Swedish voice role with fixed reference/dialogue seeds `302028`/`302128`; it is not borrowed from another cast member.
...
`rigsregnskabet-v1.png` is the alpha-cleaned Bana 5 boss: a flying tax archive assembled from confiscated hulls, ledger drums, seal presses, chains, coin rings and two revision arms around a cyan bookkeeping core. `rigsregnskabet-phase3-v1.png` adds intact and broken ledger drums plus sealed and exposed core states; `rigsregnskabet-power-rings-v1.png` adds alternating amber and cyan mechanical power halos; `rigsregnskabet-death-effects-v1.png` adds a four-step breach, rupture, core burst and afterburst. Every source sheet and exact built-in generation prompt is retained beside its alpha-cleaned runtime sheet. The pack downsamples these to named sprites; deterministic code-drawn fallbacks remain available when an asset is absent.

`tithe-archive-background-source-v1.png` is the generated top-down hanging archive used as a continuously scrolling Bana 5 backdrop. `tithe-machinery-source-v1.png` preserves the green-screen generation and `tithe-machinery-v1.png` is its alpha-cleaned 3x2 runtime source. The pack extracts dedicated chain-lock, coin-mine, customs-gate, confiscation-cabinet and interest-press sprites. `tithe-seal-intact-v2.png` and `tithe-damage-projectiles-v1.png` provide one coherent health-driven sealing-wall family: substantial intact plate, damaged plate and critically ruptured plate. The damage sheet also adds a two-frame brass/cyan register-press projectile, replacing the orange damage slash and reused blue streak; the remaining cyan press guide lines were removed. Exact built-in prompts are stored beside the assets.
...
`tithe-route-customs-v1.png` supplies the royal Tullpass warning arch, Revision and Kedjehall route emblems, and the amber/cyan mechanical register splitter. They are presentation layers over the existing authoritative gate opening and route-choice state, so collision and choice boundaries are unchanged. The untouched green-screen source and exact reference-guided built-in prompt are retained beside the production alpha PNG.
...
`copenhagen-frederik-eye-v1.png` supplies coherent phase and wreck families for Frederik Null and Øresunds Øje. `copenhagen-royal-armada-v1.png` supplies the Dannebrog compiler portal and node craft, matched intact/broken Absalon and Elefanten frigates, and Christian's intact/broken superfregatt. `copenhagen-radio-portraits-v1.png` gives Frederik, the sentient Eye, Absalon and Elefanten dedicated neutral/speaking radio pairs instead of reusing Rigsregnskabet. Dynamic blades, lenses, shield walls, turrets, boss sections and hitboxes remain state-owned overlays. The three untouched chroma-key sources and exact built-in prompts are preserved in `copenhagen-midgame-assets-v1.prompt.md`.

`copenhagen-space-details-v1.png` replaces the midgame's conspicuous moving primitives: Frederik's register blade and seized/free support craft, Eye lenses, Absalon wall nodes and turrets, repair/royal drones, Sören support and superfregatt cross locks. `copenhagen-ground-details-v1.png` adds physical Holmen braziers, intact/broken Rosenborg cores, the Memory Machine, intact/broken Korrektorius pens, saga lions and Christian's claim circuit. `copenhagen-combat-effects-v1.png` replaces the most conspicuous geometric effects with a mechanical quadrant lattice, white/corrupted tethers, Copenhagen projectiles, a shield fracture and a hit pulse; hitboxes, timing, warning borders, targeting guides and health bars remain deterministic code effects. The Eye hull alone retains its generated aspect ratio rather than being stretched to its wider prediction hit field. Exact built-in prompts and untouched chroma-key sources are preserved in `copenhagen-detail-pass-v1.prompt.md`.
...
Köpenhamns Ring owns a ten-track ACE-Step suite before the existing Codex clock: `kopenhamns-ring`, `frederik-null`, `oresunds-oje`, `kungliga-armadan`, `christians-superfregatt`, `kopenhamn-landning`, `holmen-under-staden`, `argentum-legender`, `konung-christians-vrede` and `kopenhamn-silvergryning`. The fixed-seed masters are preserved beside exact whole-bar `-loop-v1.wav` runtime edits. A shared descending four-note crown code and three-note silver pulse bind the distinct baroque-techno, optical, naval, action-RPG, law-boss and victory roles together. Level state, ground-room restore and developer fixtures select the appropriate role; the half-second mixer crossfade remains unchanged. Exact prompts, task IDs, accepted seeds, checksums and the rejected first Eye take are recorded under `music/prompts/` and `music/generation-manifest.json`.
...
Regenerate the Tithe World model family with `/home/nichlas/ai/stable-audio-3/.venv/bin/python tools/stormakt3020/generate_tithe_sfx.py`. Pass `--only NAME` one or more times to replace selected effects without regenerating the entire family. Exact prompts, seeds and durations live in that script and are summarized in `sfx/tithe-world-sfx-v1.prompt.md`; untouched raw masters remain under `sfx/raw/` beside the converted 48 kHz runtime files. Salvdirektören and Magnetbredsidan use shorter v2 generations and runtime gains `0.06` and `0.08` so repeated fire cannot dominate music or radio.
