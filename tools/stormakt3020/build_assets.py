...
def append_rts_orbit_moon(entries: list[tuple[str, Image.Image]], source: Image.Image) -> None:
    moon = trim_alpha(chroma_alpha(source.convert("RGBA")))
    moon.thumbnail((400, 200), Image.Resampling.LANCZOS)
    entries.append(("rts_orbit_forest_moon", moon))
...
def append_dungeon_karl_hammer(entries: list[tuple[str, Image.Image]],
    ...
    for source, names, target, canvas_size in [
        ...
        for index, name in enumerate(names):
            ...
            # Movement poses fit inside their regular cells. Bleeding those
            # crops pulled feet and hair from the neighbouring rows into the
            # north sprites; the stray fragments then inflated the alpha box
            # and shrank Karl himself. Attack poses intentionally extend into
            # their gutters, so only that sheet retains overlap.
            bleed = 0 if source is movement else 24
...
def append_dungeon_temple_seal_fog(entries: list[tuple[str, Image.Image]], source: Image.Image) -> None:
    for index in range(4):
        ...
        bottom = (row + 1) * source.height // 2 - 4
        sprite = trim_alpha(source.crop((left, top, right, bottom)).convert("RGBA"))
        sprite.thumbnail((54, 140), Image.Resampling.LANCZOS)
        entries.append((f"dungeon_temple_seal_fog_{index}", sprite))
...
def append_oresund_armored_train(entries: list[tuple[str, Image.Image]], source: Image.Image) -> None:
    definitions = [
        ("oresund_train_locomotive", 0, 0, (58, 96)),
        ("oresund_train_cannon", 1, 0, (60, 88)),
        ("oresund_train_command", 0, 1, (58, 84)),
        ("oresund_train_wreck", 1, 1, (88, 76)),
    ]
    for name, column, row, target in definitions:
        left = column * source.width // 2
        top = row * source.height // 2
...
def append_oresund_guard_control(entries: list[tuple[str, Image.Image]], source: Image.Image) -> None:
    definitions = [
        ("oresund_bridge_guard", 0, 0, (30, 34)),
        ("oresund_bridge_guard_aim", 1, 0, (30, 36)),
        ("oresund_control_house", 0, 1, (38, 34)),
        ("oresund_control_house_disabled", 1, 1, (38, 34)),
    ]
    for name, column, row, target in definitions:
        left = column * source.width // 2
        top = row * source.height // 2
...
def append_tithe_world_assets(
    ...
    damage_definitions = [
        ...
        ("tithe_seal_segment_critical", 1, 0, (58, 25)),
        ("tithe_effect_press_bolt_a", 0, 1, (8, 16)),
        ("tithe_effect_press_bolt_b", 1, 1, (8, 16)),
    ]
    for name, column, row, target in damage_definitions:
        cell = damage_projectiles.crop((
            column * damage_projectiles.width // 2,
            row * damage_projectiles.height // 2,
            (column + 1) * damage_projectiles.width // 2,
            (row + 1) * damage_projectiles.height // 2,
        )).convert("RGBA")
        sprite = trim_alpha(cell)
        sprite.thumbnail(target, Image.Resampling.LANCZOS)
        entries.append((name, sprite))
...
def append_snapphane_route_assets(entries: list[tuple[str, Image.Image]], source: Image.Image) -> None:
    definitions = [
        ("snapphane_rescue_corvette", 0, 0, (52, 58)),
        ("snapphane_rescue_cutter", 1, 0, (52, 58)),
        ("snapphane_rescue_buoy", 2, 0, (34, 36)),
        ("snapphane_rescue_buoy_broken", 3, 0, (36, 38)),
        ("snapphane_route_press_left", 0, 1, (72, 62)),
        ("snapphane_route_press_right", 1, 1, (72, 62)),
        ("snapphane_route_elite_hunter", 2, 1, (48, 60)),
        ("snapphane_route_release_pulse", 3, 1, (42, 42)),
    ]
    for name, column, row, target in definitions:
        cell = source.crop((
            column * source.width // 4,
...
def append_copenhagen_royal_ground_bosses(
    entries: list[tuple[str, Image.Image]], saga_source: Image.Image, wrath_source: Image.Image
) -> None:
    saga = [
        ("cph_saga_king_phase_0", 0, 0, (78, 92)),
        ("cph_saga_king_phase_1", 1, 0, (82, 92)),
        ("cph_saga_king_phase_2", 2, 0, (84, 92)),
        ("cph_saga_king_phase_3", 3, 0, (120, 82)),
        ("cph_saga_king_hit", 0, 1, (82, 92)),
        ("cph_saga_king_fallen", 1, 1, (98, 58)),
        ("cph_saga_crown_stamp", 2, 1, (48, 48)),
        ("cph_saga_rocket", 3, 1, (22, 42)),
    ]
    wrath = [
        ("cph_wrath_phase_0", 0, 0, (88, 104)),
        ("cph_wrath_phase_1", 1, 0, (92, 104)),
        ("cph_wrath_phase_2", 2, 0, (94, 104)),
        ("cph_wrath_phase_3", 3, 0, (102, 104)),
        ("cph_wrath_vulnerable", 0, 1, (90, 104)),
        ("cph_wrath_hit", 1, 1, (94, 104)),
        ("cph_wrath_fallen", 2, 1, (108, 64)),
        ("cph_wrath_claim_anchor", 3, 1, (40, 54)),
    ]
    for source, sprites in ((saga_source, saga), (wrath_source, wrath)):
        for name, column, row, target in sprites:
            left = column * source.width // 4
            top = row * source.height // 2
            right = (column + 1) * source.width // 4
            bottom = (row + 1) * source.height // 2
