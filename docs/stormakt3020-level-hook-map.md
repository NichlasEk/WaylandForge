...
## Köpenhamn dispatch - checkpoint 5 playable
...
`WAYLANDFORGE_STORMAKT_COPENHAGEN_GATE_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_ADMIRALTY_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_FREDERIK_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_EYE_TEST=1` and `WAYLANDFORGE_STORMAKT_COPENHAGEN_DANNEBROG_TEST=1` provide shortened deterministic fixtures for the five current encounters. A direct Eye fixture reached the live Dannebrog encounter without a menu frame. Two complete wide Dannebrog runs and two complete legacy runs matched within each resolution. The same 300-frame Dannebrog trace produced an identical framebuffer hash through v2 raw, PACKRLE and shared memory.
