...
## Köpenhamn dispatch - checkpoint 4 playable
...
`WAYLANDFORGE_STORMAKT_COPENHAGEN_GATE_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_ADMIRALTY_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_FREDERIK_TEST=1` and `WAYLANDFORGE_STORMAKT_COPENHAGEN_EYE_TEST=1` provide shortened deterministic fixtures for the four current encounters. Two complete wide Eye runs and two complete legacy Eye runs matched within each resolution. A direct Frederik fixture reached the live Eye encounter without a menu frame. The same 260-frame Eye trace produced an identical framebuffer hash through v2 raw, PACKRLE and shared memory.
