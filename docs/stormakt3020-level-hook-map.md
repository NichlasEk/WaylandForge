...
EXT3 uses `pointer_driver = "stormakt_rts"` and `WAYLANDFORGE_STORMAKT_POINTER=1`. In this opt-in mode the legacy stdio step grows from five to 21 bytes: `S`, Saturn buttons, pointer X/Y, pointer buttons and inside flag. Left drag selects arbitrary friendly units; right click distributes the selected set into a stable three-column formation. A direct extended-protocol capture verifies the selection rectangle. All other stdio cores continue receiving the original five bytes.
