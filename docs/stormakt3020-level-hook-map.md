...
## Köpenhamn dispatch - planned

Campaign row 7 will own level id `6`, reset seed `3707` and a separate `CopenhagenWorldState`. It is a two-act final: a level-owned shmup boss chain through Köpenhamns ring followed by a direct, menu-free landing into a separate top-down hack-and-slash state under the city. The ground act may reuse proven dungeon movement/combat helpers, but it must use its own save namespace and may not mutate Silverkroppen's `DungeonState` or autosave. PACKRLE/raw/shared lockstep parity remains part of its deterministic acceptance gate. Full design: `docs/stormakt3020-level-07-kopenhamns-ring-codex-argentum.md`.
