...
`WAYLANDFORGE_STORMAKT_COPENHAGEN_LANDING_TEST=1` and `WAYLANDFORGE_STORMAKT_COPENHAGEN_GROUND_TEST=1` extend the seven existing Copenhagen fixtures. Complete 760-frame landing traces repeat at `15cca66447daee8a` in wide and `a10ac2aeaf0db652` in legacy. Complete 1100-frame first-room traces repeat at `83acd7cc8dd0f335` in wide and `fb79379edd692663` in legacy. The same 360-frame ground trace produces `d90a2363b015462b` through WFEX v2 raw, PACKRLE and shared memory. An isolated schema-1 save resumes through the new submenu directly inside the cleared room with its health, position, potion, enemies and open door restored.
