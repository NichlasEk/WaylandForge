...
## Köpenhamn dispatch - checkpoint 6 playable
...
`WAYLANDFORGE_STORMAKT_COPENHAGEN_GATE_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_ADMIRALTY_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_FREDERIK_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_EYE_TEST=1`, `WAYLANDFORGE_STORMAKT_COPENHAGEN_DANNEBROG_TEST=1` and `WAYLANDFORGE_STORMAKT_COPENHAGEN_DUO_TEST=1` provide shortened deterministic fixtures for the six current encounters. A direct Dannebrog fixture reached the live duo without a menu frame. Two complete wide duo runs and two complete legacy runs matched within each resolution. The same 300-frame duo trace produced an identical framebuffer hash through v2 raw, PACKRLE and shared memory.
