...
## Köpenhamn dispatch - checkpoint 11 playable
...
The first Holmen graphics pass packs eight new environment sprites and eight new guard poses while reusing Silverkroppen's Karl combat/movement and temple floor. The environment sheet uses measured crop gutters so the furnace and debris remain separate. Guard movement, telegraph, contact, hit, fall and dead frames are state-driven; `FacingLeft` updates only while alive, preventing a corpse from flipping when Karl crosses it. Bana 7 now opens a six-row Copenhagen submenu in developer mode: new ring, direct landing, saved Holmen arsenal, direct Rosenborg forecourt, then two visibly locked future depths. Both 400x280 and 320x224 keep all rows and footer inside the panel.

`WAYLANDFORGE_STORMAKT_COPENHAGEN_LANDING_TEST=1` and `WAYLANDFORGE_STORMAKT_COPENHAGEN_GROUND_TEST=1` extend the seven existing Copenhagen fixtures. Complete stable-legend traces repeat at `5e4362114e11e6ef` in wide and `c01d8281af9c145b` in legacy; exaggerated traces repeat at `6844edd402cec699` and `924f9999d803cac4`. A live corrupted-legend frame produces `c9bd44c0e2b8d950` through WFEX v2 raw, PACKRLE and shared memory. Schema 1/2/3 remain accepted; an isolated schema-4 save resumes through the submenu inside the memory machine with blade form, health, position, potion, enemies and the in-flight corrupted legend restored.
