...
## Köpenhamn dispatch - checkpoint 1 playable

Campaign row 7 owns developer-startable level id `6`, reset seed `3707` and a separate `CopenhagenWorldState`. Checkpoint 1 adds an independent render/step dispatch, reads the persistent Bana 5 loadout and previous campaign support, and implements the approach plus Trekroners lock. Its wall, crossfire and repair crowns create distinct behavior, and destroying all three reaches an explicit Amiralitetet checkpoint card. `WAYLANDFORGE_STORMAKT_COPENHAGEN_GATE_TEST=1` provides a shortened deterministic gate fixture. Repeated wide and legacy runs matched within each resolution, while v2 raw, PACKRLE and shared-memory runs decoded to the same tested frame hash.
