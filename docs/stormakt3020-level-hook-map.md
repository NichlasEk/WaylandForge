...
## Köpenhamn dispatch - checkpoint 8 playable
...
`WAYLANDFORGE_STORMAKT_COPENHAGEN_LANDING_TEST=1` and `WAYLANDFORGE_STORMAKT_COPENHAGEN_GROUND_TEST=1` extend the seven existing Copenhagen fixtures. Complete 760-frame landing traces repeat at `d898dff6fb9fce95` in wide and `2f9b66426b99db0c` in legacy. Complete 1100-frame first-room traces repeat at `4ed6d056e74c424f` in wide and `a9678186e5daf7f2` in legacy. The same 360-frame ground trace produces `ff88308022e688de` through WFEX v2 raw, PACKRLE and shared memory. An isolated schema-1 save resumed directly inside the cleared room with its health, position, potion, enemies and open door restored.
